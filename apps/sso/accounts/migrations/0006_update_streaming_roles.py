...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            sso_center_role = orm.ApplicationRole.objects.get(application__uuid=settings.APP_UUID, role__name='Center')
    
            for user in User.objects.filter(application_roles=sso_center_role):
                
                if orm.Organisation.objects.filter(email__iexact=user.email).exists():
                    app_roles_dict_array = user.default_dharmashop_roles
                    app_roles_dict_array += [{'uuid': 'c362bea58c67457fa32234e3178285c4', 'roles': ['Center', 'User']}]  # Streaming 
                    app_roles_dict_array += user.default_wiki_roles
                    user.add_roles(app_roles_dict_array)
                    
                else:
                    print "NO center: ", user.username

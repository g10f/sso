...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        old_table_name = db.shorten_name(u'accounts_user_organisations2')
        new_table_name = db.shorten_name(u'accounts_user_organisations')
        fk_table_name = db.shorten_name(u'organisations_organisation')
        db.rename_table(old_table_name, new_table_name)
        try:
            sql = db.foreign_key_sql(new_table_name, 'organisation_id', fk_table_name, 'id')
            #db.execute(sql)
        except Exception, e:
            logger.exception(e)
    ...
    def backwards(self, orm):
        ...
        new_table_name = db.shorten_name(u'accounts_user_organisations')
        fk_table_name = db.shorten_name(u'organisations_organisation')
        db.rename_table(new_table_name, old_table_name)
        try:
            sql = db.foreign_key_sql(old_table_name, 'organisation_id', fk_table_name, 'id')
            #db.execute(sql)
        except Exception, e:
            logger.exception(e)

...
class AppRolesApiTests(OAuth2BaseTestCase):
    ...
    def test_app_role_with_non_existing_user(self):
        ...
        # Non-existing user
        user_app_role_error_url = self.get_url_from_api_home('user_app_role', user_id=uuid.uuid4(), app_id=data['app_id'], role=data['role'])
        user_app_role = self.client.get(user_app_role_error_url, HTTP_AUTHORIZATION=authorization).json()
        self.assertIn('error', user_app_role, user_app_role_error_url)
        self.assertEqual('not_found', user_app_role['error'], user_app_role_error_url)
        self.assertIn("No user found matching the query", user_app_role['error_description'], user_app_role_error_url)

        user_app_role = self.client.put(user_app_role_error_url, HTTP_AUTHORIZATION=authorization).json()
        self.assertIn('error', user_app_role, user_app_role_error_url)
        self.assertEqual('not_found', user_app_role['error'], user_app_role_error_url)
        self.assertIn("No user found matching the query", user_app_role['error_description'], user_app_role_error_url)

        user_app_role = self.client.delete(user_app_role_error_url, HTTP_AUTHORIZATION=authorization).json()
        self.assertIn('error', user_app_role, user_app_role_error_url)
        self.assertEqual('not_found', user_app_role['error'], user_app_role_error_url)
        self.assertIn("No user found matching the query", user_app_role['error_description'], user_app_role_error_url)

    def test_app_role_with_non_existing_app(self):
        # get token with 'role' scope
        authorization = self.get_authorization_with_client_credentials(client_id='b740653ccaa14feba7e223c609896672', scope="openid role")

        # Non-existing app
        user_app_role_error_url = self.get_url_from_api_home('user_app_role', user_id=data['user_id'], app_id=uuid.uuid4(), role=data['role'])
        user_app_role = self.client.get(user_app_role_error_url, HTTP_AUTHORIZATION=authorization).json()
        self.assertIn('error', user_app_role, user_app_role_error_url)
        self.assertEqual('not_found', user_app_role['error'], user_app_role_error_url)
        self.assertIn("Application matching query does not exist.", user_app_role['error_description'], user_app_role_error_url)
    ...
    def test_app_role_with_non_existing_role(self):
        # get token with 'role' scope
        authorization = self.get_authorization_with_client_credentials(client_id='b740653ccaa14feba7e223c609896672', scope="openid role")

        # Non-existing role
        user_app_role_error_url = self.get_url_from_api_home('user_app_role', user_id=data['user_id'], app_id=data['app_id'], role='Dummy')
        user_app_role = self.client.get(user_app_role_error_url, HTTP_AUTHORIZATION=authorization).json()
        self.assertIn('error', user_app_role, user_app_role_error_url)
        self.assertEqual('not_found', user_app_role['error'], user_app_role_error_url)
        self.assertIn("Application role not found", user_app_role['error_description'], user_app_role_error_url)

        user_app_role = self.client.put(user_app_role_error_url, HTTP_AUTHORIZATION=authorization).json()
        self.assertIn('error', user_app_role, user_app_role_error_url)
        self.assertEqual('not_found', user_app_role['error'], user_app_role_error_url)
        self.assertIn("Role Dummy not found.", user_app_role['error_description'], user_app_role_error_url)

        response = self.client.delete(user_app_role_error_url, HTTP_AUTHORIZATION=authorization)
        self.assertEqual(response.status_code, 204, user_app_role_error_url)
